...
```
components/Common/
├── DatabaseTable.vue              # 主组件入口
├── DatabaseTable.logic.ts         # 核心逻辑（composables 整合）
├── DatabaseTable.types.ts         # 类型定义
├── DatabaseTable.style.scss       # 样式
│
├── DatabaseTable.composables/     # 可组合函数
│   ├── useDatabaseTableStructure.ts   # 表结构管理
│   ├── useDatabaseTableData.ts        # 数据管理
│   ├── useDatabaseTableColumns.ts     # 列配置管理
│   ├── useDatabaseTableForm.ts        # 表单管理
│   ├── useDatabaseTableActions.ts     # 操作管理
│   └── useDatabaseTableState.ts       # 状态管理
│
└── DatabaseTable.components/      # 子组件
    ├── Toolbar.vue                # 工具栏
    ├── Content.vue                # 表格主体
    ├── Pagination.vue             # 分页器
    ...
    ├── ColumnWidthSettings.vue    # 列宽设置对话框
    └── HistoryDialog.vue          # 操作历史对话框
```
